...
def clear_state_program():
    program = Seq(
        [
            App.globalPut(Bytes("reserve"), App.globalGet(Bytes("reserve")) + App.localGet(Int(0), Bytes("balance"))),
            Return(Int(1))
        ]
    )
...
if __name__ == "__main__":
    with open("asset_approval.teal", "w") as f:
        compiled = complieTeal(approval_program(), mode=Mode.Application, version=2)
    with open("asset_clear_state.teal", "w") as f:
        compiled = complieTeal(clear_state_program(), mode=Mode.Application, version=2)
